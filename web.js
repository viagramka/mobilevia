    ...
    if (!linkedAccountsSection) {
        ...
        if (profileModalContent) {
            ...
            linkedAccountsSection.innerHTML = `
                ...
                <div class="flex gap-8 mt-16" id="googleLinkActions">
                    ${isLinked ?
                        `<button class="modal-btn secondary" onclick="unlinkGoogle()">Отвязать Google</button>` :
...
auth.onAuthStateChanged(async (user) => {
    ...
    function createCheckmarkSVG() {
        return `<svg class="verified-badge" width="16" height="16" viewBox="0 0 16 16" style="margin-left:4px;"><path d="M8 0C3.5888 0 0 3.5888 0 8C0 12.4112 3.5888 16 8 16C12.4112 16 16 12.4112 16 8C16 3.5888 12.4112 0 8 0ZM11.4712 5.528L7.528 9.4712C7.2376 9.7616 6.7624 9.7616 6.472 9.4712L4.528 7.5272C4.2376 7.2368 4.2376 6.7616 4.528 6.4712C4.8184 6.1808 5.2936 6.1808 5.584 6.4712L7 7.8872L10.416 4.4712C10.7064 4.1808 11.1816 4.1808 11.472 4.4712C11.7624 4.7616 11.7624 5.2368 11.472 5.5272L11.4712 5.528Z" fill="#2196F3"/></svg>`;
    }
            ...
            friendIds.forEach(friendId => {
                db.ref("users/" + friendId).once("value").then(userSnap => {
                    ...
                    if (user.isBot) { 
                        ...
                        if (user.verified === 1) badge = createCheckmarkSVG();
                        else if (user.verified === 2) badge = '<div class="zgs-badge"><span class="zgs-circle">ZGS</span></div>';
                    }
                ...
                div.innerHTML = `
                    ...
                    <div class="chat-info">
                        <div class="chat-name">
                            ...
                            ${channel.verified === 2 ? '<div class="zgs-badge"><span class="zgs-circle">ZGS</span></div>' : ''}
                        </div>
...
function showSubscriptions() {
    ...
    // Обновляем UI
    channelsTitle.textContent = "Мои подписки";
...
navItems.forEach(item => {
    ...
    function appendMessage(msg, firebaseKey = null) {
        ...
        // NEW: Comments button for channels
        let commentsHtml = '';
        if (currentChat && currentChat.type === 'channel') {
        ...
        const avatarHtml = msg.senderId !== currentUser?.uid ? `` : '';

        div.innerHTML = `
            
            <div class="content">
                ${msg.senderId !== currentUser?.uid ? `<div class="sender" onclick='showNotification("Хозяин его зовут ${sanitize(msg.senderName)}", 'warning');'>${sanitize(msg.senderName)}</div>` : ''}
                ${content}
                ${reactionsHtml}
                ${commentsHtml}
    ...
    function openChat(type, id, name, avatar) {
		...
		document.getElementById('.message-input-wrapper').style.display = 'none';
        ...
        if (type === 'friend') {
            ...
            currentChatRef.limitToLast(50).once("value", snapshot => {
                const loadingEl = document.getElementById('loading');
                if (loadingEl) loadingEl.remove();
                snapshot.forEach(child => appendMessage(child.val(), child.key));
                scrollToBottom();
    ...
    function showContextMenu(e, message) {
        ...
        menu.innerHTML = `
            ...
            <div class="context-menu-item" onclick="replyToMessage()">↩️ Ответить</div>
            ${isMyMessage ? `
                <div class="context-menu-item" onclick="editMessage()">✏️ Редактировать</div>
                <div class="context-menu-item" onclick="deleteMessage()">🗑️ Удалить</div>
            ` : ''}
        `;
    ...
    function showChatInfo() {
        ...
        if (isFriend) {
            db.ref("users/" + currentChat.id).once("value").then(snap => {
                ...
                chatInfoContent.innerHTML = `
                    <div style="text-align:center;">
                        <img src="${user.avatar || DEFAULT_AVATAR}" style="width:100px; height:100px; border-radius:50%; object-fit: cover;">
                        <h3>${sanitize(user.nickname)}</h3>
                        <p>${user.description || ''}</p>
                        <p>Статус: ${user.online ? '🟢 online' : '⚪ offline'}</p>
                        ${verifiedBadge}
                    </div>
            ...
            db.ref("channels/" + currentChat.id).once("value").then(snap => {
                ...
                else if (channel.verified === 2) verifiedBadge = `<div class="zgs-badge" onclick="window.open('https://zoro-game.store/')">ZGS</div>`;

                const isGroup = channel.type === 'group';
                chatInfoContent.innerHTML = `
                    <div style="text-align:center;">
                        <img src="${channel.avatar || DEFAULT_AVATAR}" style="width:100px; height:100px; border-radius:50%; object-fit: cover;">
                    ...
                    ${channel.createdBy === currentUser.uid ?
                        `<button class="modal-btn" onclick="updateParamsInProfileSettings();">Настройки</button>
...
photoInput.addEventListener('change', (e) => {
    ...
    // ========== AUTH ==========
    showRegisterBtn.addEventListener('click', (e) => { e.preventDefault(); hideModal(authModal); showModal(registerModal); });
