...
if (msg.audioUrl) {
    ...
    content += `
        <div class="voice-message" onclick="playCloudinaryAudio(this, '${msg.audioUrl}', ${msg.duration || 0})">
            <button class="play-btn">${audioIcon}</button>
            <div class="progress-bar"><div class="progress" style="width:0%"></div></div>
            <div class="duration">${Math.floor(msg.duration || 0)}s</div>
            ${isCloudinary ? '<div class="quality-badge" style="position:relative; margin-left:8px;">☁️</div>' : ''}
        </div>
    `;
}
...
document.getElementById('emailLinkLoginBtn').addEventListener('click', () => {
    const section = document.getElementById('emailLinkSection');
    if (section.style.display === 'none') {
        section.style.display = 'block';
        document.getElementById('emailLinkInput').focus();
    } else {
...
document.getElementById('sendEmailLinkBtn').addEventListener('click', async () => {
    ...
    try {
        ...
        showNotification(`Ссылка для входа отправлена на ${email}`, "success");
...
function showEmailLinkSentMessage(email) {
    ...
    messageDiv.innerHTML = `
        <svg width="24" height="24" viewBox="0 0 24 24" fill="white" style="margin-bottom: 8px;">
            <path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/>
        </svg>
        <p>Ссылка для входа отправлена на</p>
        <p><strong>${email}</strong></p>
        <p style="font-size: 12px; margin-top: 8px;">Проверьте папку "Спам", если письмо не пришло</p>
    `;
...
async function completeEmailLinkSignIn(email) {
    try {
        ...
        if (!snap.exists()) {
            ...
            showNotification("Профиль создан автоматически", "info");
        }
        ...
        <div class="modal-content" style="max-width: 400px;">
            ...
            <div style="text-align: center; margin: 20px 0;">
                <div class="voice-message-player" style="justify-content: center;">
                    ...
                    <div class="duration" id="previewDuration">0:00 / 0:00</div>
                </div>
      ...
      notification.innerHTML = `
        <div style="display: flex; align-items: center; gap: 12px;">
          ...
          <div>
            ...
            <p style="margin: 4px 0 0; font-size: 12px; color: var(--text-secondary);">${message}</p>
          </div>
        </div>
        <button onclick="this.closest('.notification').remove(); localStorage.setItem('addToHomeScreenShown', 'true');" style="background: none; border: none; color: var(--text-secondary); cursor: pointer;">
          ✕
        </button>
      `;
